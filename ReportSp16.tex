...
\begin{equation}
\begin{split}
E(g_{k}) = E(0) + \sum_{k}\bigg(\frac{\partial E}{\partial q_{k}}\bigg)q_{k} + \frac{1}{2}\sum_{j,k}q_{j} \bigg(\frac{\partial ^{2} E}{\partial q_{j} \partial q_{k}}\bigg) q_{k} + \frac{1}{6}\sum_{j,k} q_{j} \bigg(\frac{\partial ^{3}E}{\partial q_{j} \partial q_{k} \partial q_{l}} \bigg) q_{k} q_{l} + \cdots
...
\begin{equation}
\begin{split}
E(g_{k}) = E(0) + \sum_{k}g_{k} + \frac{1}{2}\sum_{j,k}q_{j} H_{j,k} q_{k} + \frac{1}{6}\sum_{j,k} q_{j} A_{j,k,l} q_{k} q_{l} + \cdots
\label{equation:TaylorElec2}
\end{split}
\end{equation}

Where the gradient $g_{k}$ is defined by the first geometric derivative, the Hessian matrix is given as $H_{j,k}$ and the anharmonicity $A_{jkl}$ is defined by the third geometric derivative with respect to $q_{k}$ in Equation \ref{equation:TaylorElec2}.

% explain the matrix and diagonalization of that matrix briefly to illustrate the method.  Then introduce the frequency intensity equation.  This allows us to quickly compare it with the BOMD method for obtaining the same data. 
